package beans.activities.dailyactivities;

import beans.activities.dailyactivities.DailyActivity.DailyActivities;

/**
 * A Go to bed representation extending {@link DailyActivity} with a name as {@link DailyActivities} and a score 
 * 
 * @param timestamp
 *            The timestamp of the activity (non-)detection as String
 * @param score
 *            The computed score of the daily go to bed (between 0 and 1)
 * @see DailyActivity
 * @author Antoine Riché
 * @since 06/14/17
 */
public class GoToBed extends DailyActivity {

	/**
	 * GoToBed constructor
	 * 
	 * @param timestamp
	 *            The timestamp of the activity (non-)detection as String
	 * @param score
	 *            The computed score of the daily go to bed (between 0 and 1)
	 */
	public GoToBed(String timestamp, float score) {
		super(timestamp, score);
		this.setName(DailyActivities.GoToBed);
	}

}
